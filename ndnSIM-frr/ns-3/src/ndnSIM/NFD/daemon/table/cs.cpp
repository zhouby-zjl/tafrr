...
Cs::const_iterator
Cs::findImpl(const Name& name) const
{
  if (!m_shouldServe || m_policy->getLimit() == 0) {
    return m_table.end();
  }

  auto range = findPrefixRange(name);
  auto match = std::find_if(range.first, range.second,
                            [&name] (const auto& entry) { return entry.canSatisfy(name); });

  if (match == range.second) {
    NFD_LOG_DEBUG("find " << name << " no-match");
    return m_table.end();
  }
  NFD_LOG_DEBUG("find " << name << " matching " << match->getName());
  m_policy->beforeUse(match);
  return match;
}
