...
GenericLinkService::encodeLpFields(const ndn::PacketBase& netPkt, lp::Packet& lpPacket)
{
  ...
  shared_ptr<lp::LltcPathIdTag> lltcPathIdTag = netPkt.getTag<lp::LltcPathIdTag>();
  if (lltcPathIdTag != nullptr) {
    lpPacket.add<lp::LltcPathIdField>(*lltcPathIdTag);
  }
  else {
    lpPacket.add<lp::LltcPathIdField>(0);
  }

  shared_ptr<lp::LltcTransientTag> lltcTransientTag = netPkt.getTag<lp::LltcTransientTag>();
  if (lltcTransientTag != nullptr) {
    lpPacket.add<lp::LltcTransientField>(*lltcTransientTag);
  }
  else {
    lpPacket.add<lp::LltcTransientField>(0);
  }

  shared_ptr<lp::LltcRsgIdTag> lltcRsgIdTag = netPkt.getTag<lp::LltcRsgIdTag>();
  if (lltcRsgIdTag != nullptr) {
    lpPacket.add<lp::LltcRsgIdField>(*lltcRsgIdTag);
  }
  else {
    lpPacket.add<lp::LltcRsgIdField>(0);
  }
                                   ...
                                   const EndpointId& endpointId)
{
  ...
  if (firstPkt.has<lp::LltcRsgIdField>()) {
	  interest->setTag(make_shared<lp::LltcRsgIdTag>(firstPkt.get<lp::LltcRsgIdField>()));
  }
                               ...
                               const EndpointId& endpointId)
{
  ...
  if (firstPkt.has<lp::LltcPathIdField>()) {
	  data->setTag(make_shared<lp::LltcPathIdTag>(firstPkt.get<lp::LltcPathIdField>()));
  }

  if (firstPkt.has<lp::LltcTransientField>()) {
	  data->setTag(make_shared<lp::LltcTransientTag>(firstPkt.get<lp::LltcTransientField>()));
  }

  if (firstPkt.has<lp::LltcRsgIdField>()) {
	  data->setTag(make_shared<lp::LltcRsgIdTag>(firstPkt.get<lp::LltcRsgIdField>()));
  }
