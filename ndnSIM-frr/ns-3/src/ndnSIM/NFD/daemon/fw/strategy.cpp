                   ...
                   const FaceEndpoint& egress)
{
  ...
  if (Forwarder::extensionLltcPrefix == NULL) {
	  pitEntry->deleteInRecord(egress.face);
  } else {
	  std::string dataNameStr = data.getName().toUri(name::UriFormat::DEFAULT);
	  size_t lltcPrefixLen = Forwarder::extensionLltcPrefix->length();
	  if (dataNameStr.length() < lltcPrefixLen) {
		  pitEntry->deleteInRecord(egress.face);
	  } else {
		  if (dataNameStr.substr(0, lltcPrefixLen).compare(*Forwarder::extensionLltcPrefix) != 0) {
			  pitEntry->deleteInRecord(egress.face);
		  }
	  }
  }
...
void Strategy::sendNonPitData(const Data& data, const FaceEndpoint& egress) {
	  m_forwarder.onOutgoingData(data, egress);
}
